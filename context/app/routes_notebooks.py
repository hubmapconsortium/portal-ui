    ...
    create_workspace_response = post(
        ...
        json={
            ...
            'workspace_details': {
                ...
                'symlinks': [{'name': f'datasets/{uuid}', 'dataset_uuid': uuid} for uuid in uuids],
            },
        },
    )
...
@blueprint.route('/notebooks/entities/<entity_type>/<uuid>.ws.ipynb', methods=['POST'])
...
def entity_notebook(entity_type, uuid):
    ...
    if vitessce_conf is None or vitessce_conf.conf is None or vitessce_conf.cells is None:
        abort(404)
