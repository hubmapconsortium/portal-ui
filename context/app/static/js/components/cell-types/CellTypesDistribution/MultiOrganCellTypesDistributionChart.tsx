...
import { CellTypeDataContextProvider, useCellTypeCountData, useCellTypeDataContext, useYScale } from './hooks';
  ...
  return (
    <Stack direction="column" spacing={1}>
      ...
      <Table size="small">
        ...
        <TableBody>
          {targetCellTypes.map((count) => (
            <TableRow key={count.name}>
              <TableCell>{count.name}</TableCell>
              <TableCell>{decimal.format(count.count)}</TableCell>
              <TableCell>{percent.format(count.count / totalCellsInOrgan)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
...
function AxisLabelTooltip({ label }: { label: string }) {
  const cellTypeCountsRecord = useCellTypeDataContext();
  const cellTypeCounts = cellTypeCountsRecord[label];
  const { cellTypes } = useCellTypesContext();
  const formattedCellTypes = cellTypes.map((type) => formatCellTypeName(type));
  return <TooltipTable organ={label} cellTypes={formattedCellTypes} cellTypeCounts={cellTypeCounts} />;
}
...
function TooltipContent({ tooltipData }: { tooltipData: TooltipData<ChartData> }) {
  ...
  const cellTypeCounts = cellTypeCountsRecord[bar.data.organ];
  return <TooltipTable organ={organ} cellTypes={[cellTypeName]} cellTypeCounts={cellTypeCounts} />;
}
...
function MultiOrganCellTypeDistributionChart({ cellTypes, organs }: MultiOrganCellTypeDistributionChartProps) {
  ...
  return (
    <CellTypeDataContextProvider value={cellTypeCountsRecord}>
      <Paper sx={{ px: 2, pb: 2 }}>
        <ChartWrapper
