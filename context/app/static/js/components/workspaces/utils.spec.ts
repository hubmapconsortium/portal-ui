...
import type { WorkspaceJob, Workspace, TemplatesTypes } from './types';

// Test helpers for partial mock data
const asJobs = (data: unknown[]) => data as unknown as WorkspaceJob[];
const asWorkspaces = (data: unknown[]) => data as unknown as Workspace[];
const asTemplates = (data: Record<string, unknown>) => data as unknown as TemplatesTypes;
...
describe('mergeJobsIntoWorkspaces', () => {
  test('it should merge jobs into workspaces', () => {
    ...
    const jobs = asJobs([{ id: 42, workspace_id: 1, other_job_info: true, status: 'running' }]);
    const mergedWorkspaces = mergeJobsIntoWorkspaces(jobs, asWorkspaces(workspaces));
  ...
  test('it should error on unexpected status', () => {
    const workspaces = asWorkspaces([{ id: 1, status: 'unexpected' }]);
    const jobs = asJobs([]);
...
describe('condenseJobs', () => {
  test('it should pick one active job if available', () => {
    ...
    const job = condenseJobs(asJobs(jobs));
  ...
  test('it should error on unexpected status', () => {
    ...
    expect(() => condenseJobs(asJobs(jobs))).toThrow('Unexpected job status');
  });
});
...
describe('sortTemplates', () => {
  ...
  test('sort with multiple disabled templates and default', () => {
    ...
    const result = sortTemplates(asTemplates(unsorted));
