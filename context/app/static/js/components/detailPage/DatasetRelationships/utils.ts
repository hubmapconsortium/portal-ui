...
export function convertProvDataToNodesAndEdges(primaryDatasetUuid: string, provData?: ProvData) {
  ...
  if (provData) {
    ...
    while (queuedEntities.length > 0) {
      ...
      if (!currentEntity || currentEntity[generatePrefix('dataset_type')] === 'Publication') {
      ...
      queuedActivities.forEach((activityUUID) => {
        ...
        if (!currentActivity || currentActivity[generatePrefix('creation_action')] === 'Create Publication Activity') {
          return;
        }
